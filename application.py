...
with mysql.connector.connect(host=host,password=password,db=db,user=user,port=port) as conn:
    cursor=conn.cursor()
    cursor.execute("CREATE TABLE admin_details (admin_username varchar(30) NOT NULL,admin_email varchar(50) NOT NULL,admin_password varbinary(255) DEFAULT NULL,created_at datetime DEFAULT CURRENT_TIMESTAMP,address text,PRIMARY KEY (admin_email),UNIQUE KEY admin_email (admin_email))")
    cursor.execute("CREATE TABLE users (useremail varchar(30) NOT NULL,username varchar(30) NOT NULL,password varbinary(255) DEFAULT NULL,address text NOT NULL,created_at datetime DEFAULT CURRENT_TIMESTAMP,gender enum('Male','Female','Other') DEFAULT NULL,PRIMARY KEY (useremail))")
    cursor.execute("CREATE TABLE items (itemid binary(16) NOT NULL,item_name mediumtext NOT NULL,description longtext NOT NULL,item_cost decimal(20,4) NOT NULL,item_quantity mediumint unsigned DEFAULT NULL,item_category enum('Home appliances','Electronics','Sports','Fashion','Grocery') DEFAULT NULL,added_by varchar(50) DEFAULT NULL,created_at datetime DEFAULT CURRENT_TIMESTAMP,imgname varchar(20) DEFAULT NULL,PRIMARY KEY (itemid),KEY items_addedby (added_by),CONSTRAINT items_addedby FOREIGN KEY (added_by) REFERENCES admin_details (admin_email))")
    cursor.execute("CREATE TABLE orders (order_id int unsigned NOT NULL AUTO_INCREMENT,order_date datetime DEFAULT CURRENT_TIMESTAMP,item_id binary(16) NOT NULL,item_name varchar(255) NOT NULL,total decimal(10,2) DEFAULT NULL,payment_by varchar(30) DEFAULT NULL,PRIMARY KEY (order_id),KEY payment_by (payment_by),KEY item_id (item_id),CONSTRAINT orders_ibfk_1 FOREIGN KEY (payment_by) REFERENCES users (useremail) ON DELETE SET NULL ON UPDATE CASCADE,CONSTRAINT orders_ibfk_2 FOREIGN KEY (item_id) REFERENCES items (itemid) ON UPDATE CASCADE)")
